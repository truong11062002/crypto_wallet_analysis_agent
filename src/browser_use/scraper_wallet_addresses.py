...
from prompts.prompt import PROMPT_WALLET_ADDRESS

GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")
# 1. Wallet Addresses and Output Directory
wallet_addresses = open("wallet_addresses.txt", "r").read().splitlines()
output_directory = "etherscan_data"

# Create output directory if it doesn't exist
os.makedirs(output_directory, exist_ok=True)


async def scrape_etherscan_address(wallet_address):
    """Scrapes Etherscan data for a single wallet address."""
    try:
        # 3. Configure LLM (adjust provider, model, temperature as needed)
        llm = utils.get_llm_model(
            provider="google",  # Or "azure_openai", "google", etc.
            model_name="gemini-2.0-flash-exp",  # Or other models like "gpt-4", "gemini-2.0-flash-exp"
            temperature=0.1,  # Lower temperature for more consistent results
            api_key=GOOGLE_API_KEY,
            # ... (add base_url and api_key if needed for your provider, e.g., for Azure)
        )
        ...
        async with await browser.new_context(
            ...
            task_prompt = PROMPT_WALLET_ADDRESS.format(wallet_address=wallet_address)
            agent = Agent(
                task=task_prompt,
                llm=llm,
...
async def main():
    ...
    for address in wallet_addresses:
        ...
        await scrape_etherscan_address(address)
